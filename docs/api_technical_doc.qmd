...
def print_json(url):
  ...
  data = response.json()
  json_output = json.dumps(data[0:5], indent=4)
  print(f"```json\n{json_output}\n```")
```
...
Returns of list of geographic levels (i.e. NUTS adminstrative units in Europe) that are available to query.
...
# | echo: false
# | output: asis
print_json("https://api.mapineq.org/functions/postgisftw.get_levels/items.json")
```
...
Returns a complete list of available data sources that are available for a specified geographic level.
...
# | echo: false
# | output: asis
print_json('https://api.mapineq.org/functions/postgisftw.get_source_by_nuts_level/items.json?_level=2&limit=500')
```
_Note: Only the first five json elements are shown._
...
# | echo: false
# | output: asis
print_json('https://api.mapineq.org/functions/postgisftw.get_source_by_year_nuts_level/items.json?_year=2017&_level=2&limit=500')
```
_Note: Only the first five json elements are shown._
...
Returns a list of all geographic levels and years that contain data for the specified data source.
...
# | echo: false
# | output: asis
print_json('https://api.mapineq.org/functions/postgisftw.get_year_nuts_level_from_source/items.json?_resource=BD_HGNACE2_R3&limit=500')
```
_Note: Only the first five json elements are shown._
...
Provides a list of all filters that can be applied to a data source along with all possible values for each filter. 
...
# | echo: false
# | output: asis
print_json('https://api.mapineq.org/functions/postgisftw.get_column_values_source_json/items.json?_resource=TGS00010&source_selections={"year":"2020","level":"2","selected":[]}&limit=40')
```
_Note: Only the first five json elements are shown._
...
# | echo: false
# | output: asis
print_json('https://api.mapineq.org/functions/postgisftw.get_column_values_source_json/items.json?_resource=DEMO_R_FIND2&source_selections={"year":"2020","level":"2","selected":[]}&limit=40')
```
_Note: Only the first five json elements are shown._
...
# | echo: false
# | output: asis
print_json('https://api.mapineq.org/functions/postgisftw.get_column_values_source_json/items.json?_resource=DEMO_R_FIND2&source_selections={"year":"2020","level":"2","selected":[{"field":"unit","value":"YR"}]}&limit=40')
```
_Note: Only the first five json elements are shown._
...
# | echo: false
# | output: asis
print_json('https://api.mapineq.org/functions/postgisftw.get_x_data/items.json?_level=2&_year=2018&X_JSON={"source":"TGS00010","conditions":[{"field":"isced11","value":"TOTAL"},{"field":"unit","value":"PC"},{"field":"age","value":"Y_GE15"},{"field":"sex","value":"T"},{"field":"freq","value":"A"}]}&limit=1500')
```
_Note: Only the first five json elements are shown._
...
Retrieve data for two sources joined by location.
