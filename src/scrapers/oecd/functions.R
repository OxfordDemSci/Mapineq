...
get_data <- function(endpoint, datdir, query='/all?', overwrite=F){
  
  tryCatch({

    tprint(paste0(endpoint, '/', query))
    ...
    if(!overwrite & file.exists(datpath)){
      ...
      query <- paste0('https://stats.oecd.org/restsdmx/sdmx.ashx/GetData/', 
                      endpoint, '/', query)
...
get_xml <- function(endpoint, datdir, overwrite=FALSE){
  tryCatch({
    ...
    query <- paste0('https://stats.oecd.org/restsdmx/sdmx.ashx/GetDataStructure/',
                    endpoint)
...
get_indicators <- function(endpoint, datdir, overwrite=FALSE){
