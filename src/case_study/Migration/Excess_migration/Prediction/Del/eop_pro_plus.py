...
class EOPProPlus:
    ...
    def _prepare_design(self, df: pd.DataFrame):
        ...
        if self.cfg.pair_mode == "pair":
            self.enc_pair = OneHotEncoder(handle_unknown='ignore', drop='first', sparse_output=True).fit(feats[['pair']])
        else:
            self.enc_orig = OneHotEncoder(handle_unknown='ignore', drop='first', sparse_output=True).fit(feats[['orig']])
            self.enc_dest = OneHotEncoder(handle_unknown='ignore', drop='first', sparse_output=True).fit(feats[['dest']])
        if self.cfg.time_effect in ("month_fe","both"):
            self.enc_time = OneHotEncoder(handle_unknown='ignore', drop='first', sparse_output=True).fit(feats[['time_id']])
