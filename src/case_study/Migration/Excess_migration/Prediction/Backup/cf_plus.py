# cf_plus_core.py — Poisson FE gravity with month-year fixed effects
...
def add_pair_my(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d['month_of_year'] = d['month'].dt.month
    d['month_year'] = d['month'].dt.to_period('M').astype(str)  # e.g., "2020-01"
    return d
...
class CFPlus:
    """
    Poisson regression with pair fixed effects and month-year fixed effects (i.e. one dummy per calendar month).
    """
    def __init__(self, cfg: CFPlusConfig):
        self.cfg = cfg
        self.enc_pair = None; self.enc_my = None; self.scaler = None
        self.model = None; self.best_alpha_ = None
    ...
    def _transform(self, pairs: pd.Series, month_year: pd.Series, Xcont: np.ndarray, fit_scaler: bool=False) -> sparse.csr_matrix:
        ...
        if not sparse.issparse(Xc): Xc = sparse.csr_matrix(Xc)
    ...
    def fit(self, flows_df: pd.DataFrame, pop_df: pd.DataFrame):
        ...
        pairs = (d['orig']+'→'+d['dest']); my = d['month_year']
        ...
        if val_mask.sum()==0 or fit_mask.sum()==0:
            tmax = d.loc[train_mask,'month'].max(); cutoff = tmax - pd.offsets.DateOffset(months=6)
            val_mask = train_mask & (d['month']>cutoff); fit_mask = train_mask & (~val_mask)
        ...
        for a in iterator:
            mdl = PoissonRegressor(alpha=a, fit_intercept=False, max_iter=self.cfg.max_iter, tol=self.cfg.tol)
            mdl.fit(X_fit, y_fit)
...
def penalty_report_from_cf(cf, *, return_deviance=False, flows_df=None, pop_df=None):
    """
    ...
    Grouping:
      ...
      - beta terms   : continuous covariates (log_pop_o, log_pop_d)
    """
    # --- learned coefficients (on encoded/scaled design) ---
    ...
    n_moy  = len(cf.enc_my .categories_[0]) - 1      # drop='first'
    n_cont = len(coef) - n_pair - n_moy               # should be 2: log_pop_o, log_pop_d
    ...
    w_pair = coef[i0:i1]
    w_moy  = coef[i1:i2]
    w_cont = coef[i2:i3]   # order: [log_pop_o, log_pop_d] (after StandardScaler)

    # squared L2 norms by group (scaled space = objective space)
    ...
    # penalty value as used by sklearn (alpha/2 * ||w||^2)
    penalty_half = 0.5 * alpha * sq_total
    penalty_nohalf = alpha * sq_total

    # de-standardize continuous coefficients for interpretation
    ...
    out = {
        ...
        "sum_sq_all": sq_total,
        "penalty_value_sklearn": penalty_half,
        "penalty_value_alpha_sum": penalty_nohalf,
        "continuous_coefs_original_scale": dict(zip(
            ["beta_log_pop_o","beta_log_pop_d"], cont_coefs_original))
    }
...
def penalty_report_from_cf(cf, *, return_deviance=False, flows_df=None, pop_df=None):
    ...
    n_pair = len(cf.enc_pair.categories_[0]) - 1      # drop='first'
    n_my   = len(cf.enc_my.categories_[0]) - 1        # month-year
    n_cont = len(coef) - n_pair - n_my                # should be 2: log_pop_o, log_pop_d
    ...
    out = {
        ...
        "penalty_value_sklearn": penalty_half,
        "penalty_value_alpha_sum": penalty_nohalf,
        "continuous_coefs_original_scale": dict(zip(names_cont, cont_coefs_original))
    }

    # (optional) training deviance...
    if return_deviance:
        d = cf._build_design(flows_df, pop_df)
        d['month'] = pd.to_datetime(d['month']).values.astype("datetime64[M]")
        train_mask = d['month'] <= pd.to_datetime(cf.cfg.train_end)
