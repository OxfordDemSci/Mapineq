...
try:
    ...
    def tqdm(x, **kwargs): return x  # no-op if tqdm unavailable

# ---------- utilities (safe, NaN-guarded) ----------
...
def compute_global_moy_baseline(df: pd.DataFrame, baseline_years=(2019,)) -> pd.Series:
    ...
    s = base.groupby('moy')['flow'].sum()
    mean_val = s.mean()
    if pd.isna(mean_val) or mean_val==0:
        return pd.Series({m:1.0 for m in range(1,13)})
    S = (s/mean_val).reindex(range(1,13)).fillna(1.0)
    return S
...
def compute_global_shock_index(df: pd.DataFrame, baseline_years=(2019,)) -> pd.Series:
    ...
    d['month'] = ensure_month(d['month'])
    d['moy']   = d['month'].dt.month
    monthly    = d.groupby('month')['flow'].sum().sort_index()
    if monthly.empty:
    ...
    expected = monthly.index.to_series().dt.month.map(S).astype(float)
    if expected.isna().all(): expected = pd.Series(1.0, index=monthly.index)
    scale = monthly.mean() if monthly.mean()!=0 else 1.0
    expected = expected.values * scale / max(S.mean(), 1e-9)
    ratio = (monthly.values + 1e-9)/np.maximum(expected, 1e-9)
    H = np.log(ratio); H = np.where(np.isfinite(H), H, 0.0)
    return pd.Series(H, index=monthly.index, name='H_t')
...
class CFPlusConfig:
    ...
    baseline_years: Tuple[int,...] = (2019,)
    use_external_covid: bool = False
    standardize_continuous: bool = True
    max_iter: int = 20000      # ↑ for convergence
    tol: float = 1e-7
    progress: bool = True      # show tqdm for alpha grid

class CFPlus:
    def __init__(self, cfg: CFPlusConfig):
        self.cfg = cfg
        self.enc_pair = None; self.enc_moy = None; self.scaler = None
    ...
    def _build_design(self, flows_df: pd.DataFrame, pop_df: pd.DataFrame, covid_df: Optional[pd.DataFrame]=None) -> pd.DataFrame:
        ...
        H = compute_global_shock_index(d[['orig','dest','month','flow']], baseline_years=self.cfg.baseline_years)
        d = d.merge(H.rename('H_t'), left_on='month', right_index=True, how='left')
        if self.cfg.use_external_covid and covid_df is not None:
            c = covid_df.copy(); c['month'] = ensure_month(c['month'])
            d = d.merge(c[['month','covid_idx']], on='month', how='left'); d['covid_idx'] = d['covid_idx'].fillna(0.0)
        else:
            d['covid_idx'] = 0.0
        return d
    ...
    def fit(self, flows_df: pd.DataFrame, pop_df: pd.DataFrame, covid_df: Optional[pd.DataFrame]=None):
        ...
        pairs = (d['orig']+'→'+d['dest']); moy = d['moy']
        Xcont = d[['log_pop_o','log_pop_d','H_t','covid_idx']].values
        y = d['flow'].values
        d['month'] = ensure_month(d['month'])
        train_mask = d['month'] <= pd.to_datetime(self.cfg.train_end)
        if train_mask.sum()==0: raise ValueError("No rows in training window. Check train_end.")
        ...
        X_fit = self._transform(pairs[fit_mask], moy[fit_mask], Xcont[fit_mask], fit_scaler=True); y_fit = y[fit_mask]
        X_val = self._transform(pairs[val_mask], moy[val_mask], Xcont[val_mask]); y_val = y[val_mask]
