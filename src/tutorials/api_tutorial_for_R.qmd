...
This tutorial demonstrates how to query the Mapineq API using R. An API (Application Programming Interface) is a tool that allows us to communicate with the Mapineq database to request data.
...
```
We now have a nice searchable `data.frame` that we can use to explore find data that are available in the Mapineq database. 
