...
This tutorial demonstrates how to query the Mapineq API using Python. An API (Application Programming Interface) is a tool that allows us to communicate with the Mapineq database to request data.
...
```{python}
#| code-fold: true
# Embed JavaScript for DataTables functionality with pagination and search
js = """
...
# View data
html_table = df_content.to_html(classes="display", index=False)
display(HTML(js + html_table))
```
We now have a nice searchable DataFrame that we can use to explore data available in the Mapineq database. 
...
display(HTML(js + html_table))
```
The response includes the data that we have requested, including identifiers like geographic unit name (`geo_name`), NUTS ID (`geo`), and year (`best_year`).
