...
ALTER TABLE IF EXISTS website.catalogue_tag
    ADD CONSTRAINT catalogue_tag_descr UNIQUE (LOWER(descr));

CREATE UNIQUE INDEX catalogue_tag_descr_unique
    ON website.catalogue_tag USING btree
    ((LOWER(descr)))	
...
BEGIN
	...
	EXECUTE FORMAT (QUERY, strTag) INTO
		retArray;
	RETURN retArray;
END;
$BODY$
